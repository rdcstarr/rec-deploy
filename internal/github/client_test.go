...
// TestCreateHookSubscribesToPushAlone pins the events array, which is not a
// free-form list: GitHub validates it against what a *repository* webhook may
// hold, and one disallowed name fails the whole call with a 422. v0.16.0 added
// `repository_dispatch` — an app-only event — and took `repo add` down for every
// repository until v0.16.1 removed it.
//
// This test cannot prove GitHub accepts the array; the stub accepts anything.
// It proves rec-deploy still asks for exactly what was verified against the
// real API, so widening it is a deliberate edit with this comment attached.
func TestCreateHookSubscribesToPushAlone(t *testing.T) {
	var got map[string]any
	...
	if len(events) != 1 || events[0] != "push" {
		t.Errorf("events = %v, want [push] — every name here must list `repository` in GitHub's availability table", events)
	}
